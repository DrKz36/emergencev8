...
def test_health_check():
    ...
    try:
        ...
        print_result(success, f"Backend status: {response.status_code}", response.json() if success else None)
        return success
...
def test_auth_verification():
    ...
    try:
        ...
        success = response.status_code in [200, 401, 404]  # 404 si l'endpoint n'existe pas encore

        if response.status_code == 200:
            print_result(True, "Token valide - Utilisateur authentifié", response.json())
        elif response.status_code == 401:
            print_result(False, "Token invalide ou expiré", {"status": response.status_code, "body": response.text})
        else:
            print_result(True, f"Endpoint retourne {response.status_code} (endpoint peut ne pas exister)", {"status": response.status_code})
...
def test_chat_endpoint():
    ...
    try:
        ...
        response = requests.post(f"{BASE_URL}/api/chat", headers=HEADERS, json=payload, timeout=10)
        success = response.status_code in [200, 201, 404]

        if response.status_code in [200, 201]:
            print_result(True, "Endpoint chat accessible et fonctionnel", {
                "status": response.status_code,
                "response_preview": response.text[:200] if response.text else None
            })
        elif response.status_code == 404:
            print_result(True, "Endpoint chat non trouvé (peut utiliser un autre chemin)", {"status": 404})
        else:
...
def test_debate_endpoint():
    ...
    try:
        payload = {
            ...
            "participants": ["neo", "anima"]
        }
        response = requests.post(f"{BASE_URL}/api/debate", headers=HEADERS, json=payload, timeout=10)
        success = response.status_code in [200, 201, 404]

        if response.status_code in [200, 201]:
            print_result(True, "Endpoint debate accessible et fonctionnel", {
                "status": response.status_code,
                "response_preview": response.text[:200] if response.text else None
            })
        elif response.status_code == 404:
            print_result(True, "Endpoint debate non trouvé (peut utiliser un autre chemin)", {"status": 404})
        else:
            print_result(False, f"Erreur {response.status_code}", {"body": response.text[:200]})
...
def test_settings_security():
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/settings/security", headers=HEADERS, timeout=5)
        success = response.status_code in [200, 404]

        if response.status_code == 200:
            print_result(True, "Endpoint settings/security accessible", response.json())
        elif response.status_code == 404:
            print_result(True, "Endpoint settings/security non trouvé (normal si pas encore implémenté)", {"status": 404})
        else:
            print_result(False, f"Erreur {response.status_code}", {"body": response.text[:200]})
...
def test_cors_headers():
    ...
    try:
        ...
        print_result(success, "Headers CORS", cors_headers if success else "Pas de headers CORS trouvés")
        return success
    except Exception as e:
