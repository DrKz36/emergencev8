...
ANALYSIS_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "summary": {"type": "string", "description": "RÃ©sumÃ© concis de la conversation (2-3 phrases)."},
        "concepts": {"type": "array", "items": {"type": "string"}, "description": "3 Ã  5 concepts clÃ©s."},
        "entities": {"type": "array", "items": {"type": "string"}, "description": "EntitÃ©s nommÃ©es."},
    },
...
class MemoryAnalyzer:
    ...
    def _ensure_ready(self):
        if not self.chat_service:
            self.is_ready = False
            logger.error("DÃ©pendance 'chat_service' non injectÃ©e. L'analyse est impossible.")
            raise ReferenceError("MemoryAnalyzer n'est pas prÃªt : chat_service manquant.")
        self.is_ready = True
    ...
    async def _analyze(self, session_id: str, history: List[Dict[str, Any]], persist: bool = True, force: bool = False) -> Dict[str, Any]:
        self._ensure_ready()
        logger.info(f"Lancement de l'analyse sÃ©mantique (persist={persist}) pour {session_id}")
        ...
        try:
            ...
            primary_error = e
            logger.warning(f"Analyse (neo) a Ã©chouÃ© â€” tentative fallback nexus. err={e}", exc_info=True)
        ...
        try:
            session_manager = getattr(self.chat_service, "session_manager", None) if self.chat_service else None
            if session_manager:
            ...
            logger.debug(f"Impossible de mettre Ã  jour les mÃ©tadonnÃ©es de session {session_id}: {meta_err}")

        await self._notify(session_id, {"session_id": session_id, "status": "completed"})
        logger.info(f"Analyse sÃ©mantique terminÃ©e (persist={persist}) pour {session_id}.")
        return analysis_result

    async def analyze_session_for_concepts(self, session_id: str, history: List[Dict[str, Any]], *, force: bool = False):
