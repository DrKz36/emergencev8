...
# Mute Chroma telemetry (optionnel)
try:
...
class ChatService:
    ...
    def _get_agent_config(self, agent_id: str) -> Tuple[str, str, str]:
        ...
        # Raccourci coûts : anima -> gpt-4o-mini
    ...
    async def _process_agent_response_stream(
        ...
        """
        Stream agent :
        - Anti-doublon AVANT ws:chat_stream_start (évite UI bloquée).
        - Lock in-flight, puis stream.
        """
        ...
        try:
                        ...
                        try:
                            ...
                            asyncio.create_task(
                                run_analysis_and_notify(
                                    self.session_manager, connection_manager, session_id, agent_id, self.enable_analysis
                                )
                            )
                        except Exception:
                            pass
            ...
            ltm_block = (
                await self.memory.build_memory_context(session_id, last_user_message, top_k=5)
            ...
            if use_rag and last_user_message:
                ...
                rag_context = self.memory.merge_blocks([("Mémoire (concepts clés)", mem_block), ("Documents pertinents", doc_block)])
                try:
                    await connection_manager.send_personal_message(
            ...
            # Fallback (rate_limit/provider_error)
            try:
                if not full_response_text and cost_info_container.get("__error__") in ("rate_limit", "provider_error"):
                    if agent_id == "neo":
                        ...
                        alt_stream = self.streamer.get_llm_response_stream(
                            "anthropic", "claude-3-haiku-20240307", system_prompt, normalized_history, fallback_cost
                        )
