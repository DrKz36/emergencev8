...
class WebhookEventDispatcher:
    ...
    def __init__(self) -> None:
        self._delivery_service: Optional[Any] = (
            None  # WebhookDeliveryService (circular import)
        )
    ...
    async def emit_thread_created(
        self, thread_id: str, user_id: str, thread_data: dict[str, Any]
    ) -> None:
    ...
    async def emit_debate_completed(
        self, debate_id: str, user_id: str, topic: str, turns_count: int, synthesis: str
    ) -> None:
    ...
    async def _emit_event(
        self, event_type: WebhookEvent, user_id: str, data: dict[str, Any]
    ) -> None:
        ...
        if not self._delivery_service:
            logger.warning(
                f"Webhook delivery service not initialized, skipping {event_type}"
            )
            return
        ...
        try:
            ...
            asyncio.create_task(
                self._delivery_service.deliver_event(
                    event_type=str(event_type), user_id=user_id, payload=payload
                )
            )
            ...
            logger.error(
                f"Failed to dispatch webhook event {event_type}: {e}", exc_info=True
            )
