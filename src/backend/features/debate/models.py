...
class DebateConfig(BaseModel):
    ...
    agent_order: List[str] = Field(
        ..., min_length=3, max_length=3, description="L'ordre des agents participants."
    )
    use_rag: bool = Field(
        default=False, description="Indique si le débat doit utiliser le contexte RAG."
    )
...
class DebateTurn(BaseModel):
    ...
    agent_responses: Dict[str, str] = Field(
        default_factory=dict, description="Réponses des agents pour ce tour."
    )
...
class DebateSession(BaseModel):
    ...
    rag_context: Optional[str] = Field(
        default=None, description="Contexte RAG utilisé pour le débat, s'il y a lieu."
    )
