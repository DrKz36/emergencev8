...
class VoiceService:
    def __init__(self, config: VoiceServiceConfig, http_client: httpx.AsyncClient, chat_service: ChatService):
        self.config = config
        self.http_client = http_client
        self.chat_service = chat_service
        logger.info(f"VoiceService initialisé avec STT, TTS, et ChatService.")
    ...
    async def transcribe_audio(self, audio_stream: AsyncGenerator[bytes, None]) -> str:
        ...
        try:
            response = await self.http_client.post(url, headers=headers, data=data, files=files, timeout=60.0)
            ...
            logger.error(f"Erreur API Whisper: {e.response.status_code} - {error_body.decode()}")
            raise Exception(f"Erreur de transcription.")

    async def synthesize_speech(self, text: str) -> AsyncGenerator[bytes, None]:
        logger.info(f"Début de la synthèse vocale avec ElevenLabs pour le texte: '{text[:30]}...'")
        
        if not self.config.tts_voice_id or not isinstance(self.config.tts_voice_id, str) or self.config.tts_voice_id.startswith('sk_'):
             logger.error(f"ID de voix ElevenLabs invalide ou manquant. ID actuel : '{self.config.tts_voice_id}'. Vérifie la configuration.")
        ...
        headers = {"Accept": "audio/mpeg", "Content-Type": "application/json", "xi-api-key": self.config.tts_api_key}
        payload = {"text": text, "model_id": self.config.tts_model_id}

        try:
            async with self.http_client.stream("POST", url, headers=headers, json=payload, timeout=60.0) as response:
                if not response.is_success:
                    error_body = await response.aread()
                    logger.error(f"Erreur API ElevenLabs: {response.status_code} - {error_body.decode()}")
                    response.raise_for_status()
            ...
            raise Exception(f"Erreur de synthèse vocale (HTTP {e.response.status_code}).")
        except Exception as e:
            logger.error(f"Erreur inattendue pendant la synthèse vocale: {e}")
            raise

    async def process_voice_interaction(self, audio_stream: AsyncGenerator[bytes, None], agent_name: str, session_id: str) -> AsyncGenerator[Dict[str, Any], None]:
        """
        ...
        try:
            ...
            if not user_text:
                ...
                yield {"type": "text", "data": "Je n'ai rien entendu. Peux-tu répéter ?"}
                async for chunk in self.synthesize_speech("Je n'ai rien entendu. Peux-tu répéter ?"):
                    yield {"type": "audio", "data": chunk}
                return

            logger.info(f"Envoi du texte transcrit '{user_text}' à l'agent '{agent_name}'...")
            ...
            error_message = f"Désolé, une erreur technique est survenue lors de la génération de la réponse vocale."
            try:
                yield {"type": "text", "data": error_message}
                async for chunk in self.synthesize_speech(error_message):
                    yield {"type": "audio", "data": chunk}
            except Exception as synth_error:
                logger.error(f"Impossible de générer le message d'erreur vocal: {synth_error}")
