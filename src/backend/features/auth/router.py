...
# EMERGENCY ENDPOINT - À SUPPRIMER APRÈS UTILISATION
@router.post("/emergency/set-password")
async def emergency_set_password(
    payload: Dict[str, Any],
    auth_service: AuthService = Depends(get_auth_service),
) -> Dict[str, Any]:
    """
    Endpoint d'urgence pour définir un mot de passe.
    À SUPPRIMER après utilisation!
    Protégé par un secret d'urgence.
    """
    import os
    import bcrypt

    # Vérifier le secret d'urgence
    emergency_secret = os.getenv("EMERGENCY_PASSWORD_SECRET", "")
    provided_secret = payload.get("emergency_secret", "")

    if not emergency_secret or not provided_secret or emergency_secret != provided_secret:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not Found")
    ...
    if not email or not password:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Email et password requis")
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Mot de passe mis à jour pour {email}",
            "warning": "ENDPOINT D'URGENCE - À SUPPRIMER!"
        }
