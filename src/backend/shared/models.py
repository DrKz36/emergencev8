...
class Session(BaseModel):
    ...
    end_time: Optional[datetime] = Field(
        None, description="Horodatage de la fin de la session"
    )
    last_activity: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Horodatage de la dernière activité",
    )
    history: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Liste des messages (ChatMessage, AgentMessage) de la session",
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Metadata for the session (thread info, summaries).",
    )
...
class ChatMessage(BaseModel):
    ...
    session_id: str = Field(
        ..., description="ID de la session à laquelle le message appartient"
    )
    role: Role = Field(
        ..., description="Rôle de l'auteur du message (user ou assistant)"
    )
    agent: str = Field(
        ..., description="Agent concerné (ex: 'anima', 'neo', ou 'user')"
    )
    content: str = Field(
        ..., max_length=50000, description="Contenu textuel du message (max 50KB)"
    )
    timestamp: str = Field(..., description="Horodatage ISO 8601 du message")
    cost: Optional[float] = Field(
        None, description="Coût associé à la génération de ce message (si assistant)"
    )
    tokens: Optional[Dict[str, Any]] = Field(
        None, description="Détail des tokens utilisés (input, output)"
    )
    agents: Optional[List[str]] = Field(
        None, description="Liste des agents cibles pour un nouveau message utilisateur."
    )
    use_rag: bool = Field(
        False, description="Indique si le RAG doit être utilisé pour ce message."
    )
