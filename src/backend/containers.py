...
# V6.0 — Wire explicite des routers (debate/chat/documents) + instance module-level.
...
class ServiceContainer(containers.DeclarativeContainer):
    ...
    V6.0 : aucun changement d’arbo ; corrections DI et wiring explicite.
    """
    ...
    # --- BLOC DE SERVICES (sessions, chat, mémoire, WS) ---
    session_manager = providers.Singleton(
        SessionManager,
        db_manager=db_manager,
        # NOTE historique : l’injection manuelle du MemoryAnalyzer n’est plus nécessaire
        # depuis V5.2+ (memory_analyzer ci-dessous).
    )
    ...
    # Analyseur mémoire avec db_manager (corrigé V5.2+)
...
container.wire(modules=[
    ...
    # ajouter ici d’autres modules si vous utilisez Provide[...] ailleurs
])
