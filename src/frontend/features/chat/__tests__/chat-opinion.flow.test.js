...
test('chat opinion flow reuses request id and routes buckets to the source agent', async () => {
  await withDomStub(async () => {
    ...
    assert.equal(module._messageBuckets.get('stream-1'), streamBucket);
    ...
    module.handleStreamChunk({ agent_id: 'anima', id: 'stream-1', chunk: 'Encore' });
    bucketMessages = state.get(`chat.messages.${streamBucket}`) || [];
    ...
    const persistedOpinion = state.get(`chat.messages.${streamBucket}`)?.find((msg) => msg.id === 'db-opinion-1');
