 ...
 * V42.2 — "Nouveau débat" désactivé pendant HOLD + compte à rebours ; labels robustes.
 */
...
export class DebateUI {
  constructor(eventBus) {
    ...
    console.log('✅ DebateUI V42.2 prêt.');
  }
  ...
  render(container, debateState) {
    ...
    if (!hasHistory && !isActive) { container.innerHTML = this._renderCreateView(statusText); this._bindCreateEvents(container); return; }
    container.innerHTML = this._renderTimelineView(debateState); this._bindTimelineEvents(container, debateState);
  }
  ...
  _renderCreateView(statusText) {
    ...
    const segA = this._segAgents('attacker',defaultAttacker);
    const segB = this._segAgents('challenger',defaultChallenger);
    const segM = this._segAgents('mediator',defaultMediator);
    const roundsTabs = this._segRounds(3);
    return `
      <div class="debate-view-wrapper"><div class="card">
        <div class="card-header timeline-header">
          <div class="title-center"><div class="debate-title">Sujet du Débat</div><div class="debate-topic muted">—</div></div>
        ...
        <div class="card-body">
          <div class="debate-create-body">
            <div class="form-group form-topic">
              ...
              <textarea id="debate-topic" class="input-text" rows="4" placeholder="Ex: L’IA peut-elle développer une conscience authentique ?"></textarea>
            </div>
            <div class="form-group form-attacker"><label>Attaquant</label>${segA}</div>
            <div class="form-group form-challenger"><label>Challenger</label>${segB}</div>
            <div class="form-group form-mediator"><label>Médiateur (synthèse finale)</label>${segM}</div>
            <div class="form-group form-rounds"><label>Nombre de tours</label>${roundsTabs}</div>
            ...
            <div class="action-center"><button class="btn btn-primary button button-primary" id="debate-start">Lancer le débat</button></div>
          </div>
        </div>
      </div></div>`;
  }
  ...
  _bindCreateEvents(root) {
    ...
    root.querySelector('#rag-power')?.addEventListener('click', (e) => {
      const btn = e.currentTarget; const on = btn.getAttribute('aria-checked') === 'true';
      btn.setAttribute('aria-checked', on ? 'false' : 'true'); root.querySelector('#rag-label')?.classList.toggle('muted', !on);
    });
    this._bindTabs(root,'attacker'); this._bindTabs(root,'challenger'); this._bindTabs(root,'mediator'); this._bindTabs(root,'rounds');
    ...
    root.querySelector('#debate-start')?.addEventListener('click', () => {
      ...
      this.eventBus.emit(EVENTS.DEBATE_CREATE || 'debate:create', { topic, rounds, agentOrder:[attacker,challenger,mediator], useRag });
    });
  }
  ...
  _renderTimelineView(state) {
    ...
    const header = `
      <div class="card-header timeline-header">
        <div class="title-center"><div class="debate-title">Sujet du Débat</div><div class="debate-topic">${this._html(topic || '—')}</div></div>
        <div class="debate-status">${this._html(state?.statusText ?? '')}</div>
      </div>`;

    const turns = Array.isArray(state?.turns) ? state.turns : [];
    const body = `
      <div class="card-body"><div class="chat-messages">
        ${turns.map(t=>{
          const agentId=t.agent||'anima'; const name=agentLabel(agentId); const text=marked.parse(t.text||'');
          return `<div class="message assistant ${this._html(agentId)}"><div class="message-content">
    ...
    const synthesis = (state?.status==='completed' && state?.synthesis)
      ? `<section class="debate-synthesis">
           ...
           <div class="chat-messages"><div class="message assistant ${this._html(synthesizerId)}">
             <div class="message-content"><div class="message-meta meta-inside">
               <strong class="sender-name">${this._html(synthesizerName)}</strong></div>
               <div class="message-text"><em>${marked.parse(state.synthesis)}</em></div>
             </div></div></div></section>`
      : '';

    const footer = `
      <div class="card-footer"><div class="debate-actions">
  ...
  _bindTimelineEvents(root, state) {
    root.querySelector('#debate-export')?.addEventListener('click', () => {
      const topic=(state?.topic||'').trim(); const lines=[`# Débat — ${topic || 'Sans titre'}`];
      (state?.turns||[]).forEach(t=>{ const name=agentLabel(t.agent||'anima'); lines.push(`\n## ${name}\n\n${t.text||''}`); });
      if (state?.synthesis) {
        const synName = agentLabel(state.config?.agentOrder?.slice(-1)[0] || 'nexus');
        lines.push(`\n---\n\n### Synthèse (${synName})\n\n${state.synthesis}`);
      }
      const blob=new Blob([lines.join('\n')],{type:'text/markdown;charset=utf-8'});
      const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`debate_${Date.now()}.md`; a.click(); URL.revokeObjectURL(url);
    });
    ...
    if (btnNew) {
      const tick = () => {
        ...
        if (next===0) { btnNew.disabled=false; btnNew.removeAttribute('aria-disabled'); } else { setTimeout(tick,1000); }
      };
  ...
  _bindTabs(root, segRole) {
    const seg = root.querySelector(`[data-seg="${segRole}"]`); if (!seg) return;
    seg.addEventListener('click', (e) => {
      const btn = e.target?.closest?.('.button-tab'); if (!btn) return;
      seg.querySelectorAll('.button-tab').forEach(b => { const is=(b===btn); b.classList.toggle('active',is); b.setAttribute('aria-pressed',is?'true':'false'); });
  ...
  _autoSelectMediator(root) {
    try {
      ...
      const attacker=this._getSegValue(root,'attacker'); const challenger=this._getSegValue(root,'challenger');
      ...
      seg.querySelectorAll('.button-tab[data-value]').forEach(btn => {
        const val=btn.getAttribute('data-value'); const is=(val===mediator);
        btn.classList.toggle('active', is); btn.setAttribute('aria-pressed', is?'true':'false');
      });
  ...
  _segAgents(role,def='anima'){ const agents=['anima','neo','nexus'];
    const buttons=agents.map(a=>{ const label=agentLabel(a); const is=a===def;
      return `<button type="button" class="button-tab ${is?'active':''}" data-value="${a}" aria-pressed="${is?'true':'false'}">${this._html(label)}</button>`; }).join('');
