/**
 * @module core/app
 * @description Coeur de l'application ÉMERGENCE - V35.0 "Concordance/Mobile"
 * - Rendu de la navigation dans la sidebar (desktop) ET dans le header (mobile).
 * - Active state synchronisé entre les deux zones.
 */
...
export class App {
  constructor(eventBus, state) {
    ...
    console.log('✅ App V35.0 (Concordance/Mobile) Initialisée.');
    this.init();
  }
  ...
  init() {
    ...
    // re-render nav on resize (mobile <-> desktop)
  ...
  renderNavigation() {
    ...
    // Header (mobile) — icônes tap-friendly
    if (this.dom.headerNav) {
      if (this.isMobile()) {
        ...
        this.dom.headerNav.innerHTML = ''; // en desktop on n’affiche pas la nav header
      }
    }
  }
  ...
  async loadModule(moduleId) {
    ...
    if (!moduleLoader) { console.error(`❌ CRITICAL: Aucun chargeur pour "${moduleId}".`); return null; }
  ...
  async showModule(moduleId, isInitialLoad = false) {
    ...
    this.renderNavigation(); // sync active state (header + sidebar)
