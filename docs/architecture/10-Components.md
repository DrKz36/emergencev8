...
- **`features/chat/memory_service.py`** (V3.3.32 - Refactor Phase 2) : service spécialisé extraction mémoire consolidée depuis ChatService ; gère consolidated memory retrieval avec caching RAG (hit rate 30-40%), concept grouping par similarité sémantique (cosine > 0.7), temporal history building avec timestamps enrichis, timeline generation via MemoryQueryTool ; expose méthodes `get_consolidated_memory()`, `group_concepts_by_theme()`, `extract_group_title()`, `build_temporal_history_context()` ; injecté dans ChatService via DI (ServiceContainer).
- **`features/chat/prompt_service.py`** (V3.3.32 - Refactor Phase 3) : service spécialisé gestion prompts et configurations agents extrait depuis ChatService ; charge prompts depuis fichiers markdown avec versioning intelligent (priorité v3 > v2 > lite par nom), résout configurations agents (provider, model) depuis settings avec fallback order, applique règles de style français tutoiement via balises [STYLE_RULES], supporte ENV override coût (EMERGENCE_FORCE_CHEAP_ANIMA) ; expose méthodes `_load_prompts()`, `get_agent_config()`, `apply_style_rules()` ; injecté dans ChatService via DI.
