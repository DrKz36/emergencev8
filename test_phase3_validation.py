...
def test_memory_buttons_migrated():
    ...
    if 'data-memory-tab="history"' in js_content and 'data-memory-tab="graph"' in js_content:
...
def test_graph_buttons_migrated():
    ...
    if 'data-action="reset-view"' in js_content and 'data-action="reload"' in js_content:
...
def test_sticky_header_implemented():
    ...
    print_success(f"Sticky header properties found: {len(found_properties)}/{len(required_properties)}")
...
def test_design_tokens_available():
    ...
    if tokens_exist:
        print_success(f"design-tokens.css exists")

    if system_exist:
        print_success(f"design-system.css exists")
