...
# Costs by model
cursor.execute('SELECT model, COUNT(*), SUM(total_cost), SUM(input_tokens), SUM(output_tokens) FROM costs GROUP BY model')
print('\nCosts by Model:')
for row in cursor.fetchall():
    model, count, total_cost, input_tokens, output_tokens = row
    print(f'  {model}: {count} entries, ${total_cost:.6f}, {input_tokens} input tokens, {output_tokens} output tokens')

# Recent costs
cursor.execute('SELECT timestamp, model, total_cost, input_tokens, output_tokens FROM costs ORDER BY timestamp DESC LIMIT 5')
print('\nRecent Cost Entries:')
for row in cursor.fetchall():
