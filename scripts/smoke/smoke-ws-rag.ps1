...
$env = @{ type=$MsgType; payload=@{ use_rag=$true; agent_id=$AgentId; text=$Text } } | ConvertTo-Json -Compress
Write-Host ">> Sending envelope: $env"
...
while ($ws.State -eq [System.Net.WebSockets.WebSocketState]::Open) {
  try {
    $recvCts = [Threading.CancellationTokenSource]::new(20000)
    ...
    if ($result.Count -gt 0) {
      ...
      try {
        ...
        if ($obj.type -eq 'ws:chat_stream_end' -or $obj.type -eq 'ws:error') { break }
      } catch { }
    }
...
Write-Host ">> Done. WS State: $($ws.State)"
