...
async def test_archive_without_gardener_backwards_compat(db):
    ...
    thread = await queries.get_thread_any(db, thread_id, session_id=None, user_id="user_test")
...
async def test_archive_triggers_consolidation(db, mock_gardener):
    ...
    thread = await queries.get_thread_any(db, thread_id, session_id=None, user_id="user_123")
    assert thread is not None
    assert thread['archived'] == 1
    assert thread['archived_at'] is not None
    ...
    consolidated_dt = datetime.fromisoformat(thread['consolidated_at'].replace('Z', '+00:00'))
...
async def test_consolidation_failure_does_not_block_archiving(db, mock_gardener):
    ...
    mock_gardener._tend_single_thread = AsyncMock(side_effect=Exception("ChromaDB indisponible"))
    ...
    thread = await queries.get_thread_any(db, thread_id, session_id=None, user_id="user_fail")
    assert thread is not None
    assert thread['archived'] == 1
    assert thread['archived_at'] is not None
...
async def test_unarchive_does_not_trigger_consolidation(db, mock_gardener):
    ...
    # Archiver d'abord
    await queries.update_thread(
        db, thread_id, "sess_test", user_id="user_unarchive",
        archived=True, gardener=mock_gardener
    )
    ...
    await queries.update_thread(
        db, thread_id, "sess_test", user_id="user_unarchive",
        archived=False, gardener=mock_gardener
    )
    ...
    thread = await queries.get_thread_any(db, thread_id, session_id=None, user_id="user_unarchive")
