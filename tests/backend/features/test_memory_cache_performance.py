...
class TestMemoryCachePerformance:
    ...
    def test_cache_hit_performance(self, memory_builder, mock_vector_service):
        ...
        prefs_miss = memory_builder._fetch_active_preferences_cached(
            collection, user_id
        )
        ...
        assert duration_hit <= duration_miss, (
            f"Cache hit devrait être au moins aussi rapide (hit={duration_hit * 1000:.2f}ms, miss={duration_miss * 1000:.2f}ms)"
        )
        assert duration_hit < 0.010, (
            f"Cache hit devrait être <10ms (actual={duration_hit * 1000:.2f}ms)"
        )
        print(
    ...
    def test_cache_hit_rate_realistic_traffic(
        self, memory_builder, mock_vector_service
    ):
        ...
        assert hit_rate > 0.75, (
            f"Hit rate devrait être >75% (actual={hit_rate * 100:.1f}%)"
        )

        # First query for each user = miss (20), rest = hit (80)
        expected_hits = total - unique_users
        expected_rate = expected_hits / total
        assert hit_rate >= expected_rate * 0.9, (
            f"Hit rate proche de l'attendu ({expected_rate * 100:.1f}%)"
        )
    ...
    def test_cache_ttl_expiration(self, memory_builder, mock_vector_service):
        ...
        memory_builder._prefs_cache[user_id] = (
            prefs,
        ...
        assert (datetime.now() - new_cached_at).total_seconds() < 1, (
            "Entry devrait être rafraîchie"
        )

    def test_cache_cleanup_garbage_collection(
        self, memory_builder, mock_vector_service
    ):
        ...
        for i in range(3):
            ...
            memory_builder._prefs_cache[f"user_{i}"] = (
                prefs,
                datetime.now() - timedelta(minutes=6),
            )
        ...
        assert len(memory_builder._prefs_cache) == 2, (
            "3 entrées expirées devraient être supprimées"
        )
    ...
    def test_cache_memory_efficiency(self, memory_builder, mock_vector_service):
        ...
        assert cache_size == 100, (
            f"Cache devrait contenir 100 entrées (actual={cache_size})"
        )
    ...
    @pytest.mark.asyncio
    async def test_cache_integration_build_context(
        self, memory_builder, mock_vector_service
    ):
        ...
        start_miss = time.perf_counter()
        context_miss = await memory_builder.build_memory_context(
            session_id, user_message, top_k=5
        )
        duration_miss = time.perf_counter() - start_miss

        # Second call (cache hit)
        start_hit = time.perf_counter()
        context_hit = await memory_builder.build_memory_context(
            session_id, user_message, top_k=5
        )
        duration_hit = time.perf_counter() - start_hit
        ...
        assert (
            "python" in context_hit.lower() or "containerization" in context_hit.lower()
        )
    ...
    def mock_vector_service(self):
        ...
        collection.get = Mock(
            return_value={
                ...
                "metadatas": [
                    {"type": "preference", "confidence": 0.9, "user_id": "user_test"}
                ],
            }
        )
        vs.get_or_create_collection = Mock(return_value=collection)
