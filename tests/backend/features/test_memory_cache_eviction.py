...
class TestCacheEviction:
        ...
        with patch('backend.features.memory.analyzer.queries.update_session_analysis_data', new=AsyncMock()):
            ...
            for i in range(100):
                ...
                _ANALYSIS_CACHE[cache_key] = (
                    ...
                    base_time - timedelta(minutes=100 - i)  # Les plus récents ont i proche de 100
                )
            ...
            assert cache_size <= 55, f"Cache devrait être réduit à ~50 entrées, actuel: {cache_size}"
            assert cache_size >= 45, f"Cache ne devrait pas être trop réduit, actuel: {cache_size}"
    ...
    async def test_cache_keeps_most_recent(self, analyzer, mock_db_manager):
        ...
        with patch('backend.features.memory.analyzer.queries.update_session_analysis_data', new=AsyncMock()):
            ...
            assert recent_count >= 25, f"Devrait garder ~25+ entrées récentes, actuel: {recent_count}"

            # La plupart des anciennes devraient être supprimées
            assert old_count < 30, f"Devrait supprimer la plupart des anciennes, actuel: {old_count}"
    ...
    async def test_no_eviction_below_threshold(self, analyzer, mock_db_manager):
        ...
        with patch('backend.features.memory.analyzer.queries.update_session_analysis_data', new=AsyncMock()):
            ...
            assert len(_ANALYSIS_CACHE) == 71, f"Aucune éviction ne devrait se produire, actuel: {len(_ANALYSIS_CACHE)}"
    ...
    async def test_eviction_at_threshold(self, analyzer, mock_db_manager):
        ...
        with patch('backend.features.memory.analyzer.queries.update_session_analysis_data', new=AsyncMock()):
            ...
            cache_size = len(_ANALYSIS_CACHE)
            assert cache_size <= 55, f"Cache devrait être réduit à ~50, actuel: {cache_size}"
            assert cache_size >= 45, f"Cache ne devrait pas être sur-réduit, actuel: {cache_size}"
    ...
    async def test_cache_ttl_expiration(self, analyzer, mock_db_manager):
        ...
        with patch('backend.features.memory.analyzer.queries.update_session_analysis_data', new=AsyncMock()):
            ...
            if valid_key in _ANALYSIS_CACHE:
                ...
                assert cached_time > expired_time, "Le cache devrait contenir la nouvelle valeur"
