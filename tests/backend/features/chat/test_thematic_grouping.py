...
class TestThematicGrouping:
            ...
            return_value=np.array(
                [
                    ...
                    [
                        ...
                        0.12,
                        0.22,
                    ],  # Concept 2: Kubernetes (similaire à 1)
    ...
    async def test_clustering_similar_concepts(self, mock_vector_service):
        ...
        assert all(isinstance(g, list) for g in groups.values()), (
            "Tous les groupes doivent être des listes"
        )
        assert sum(len(g) for g in groups.values()) == 3, (
            "Tous les concepts doivent être assignés"
        )
    ...
    def test_extract_group_title_with_relevant_keywords(self):
        ...
        assert "utilisateur" not in title.lower(), (
            "Titre ne doit pas contenir 'utilisateur' (stop word)"
        )
        assert "demande" not in title.lower(), (
            "Titre ne doit pas contenir 'demande' (stop word)"
        )
        ...
        has_relevant_keyword = any(
            keyword in title_lower for keyword in relevant_keywords
        )
        assert has_relevant_keyword, (
            f"Titre '{title}' devrait contenir au moins un mot-clé pertinent"
        )
    ...
    def test_extract_group_title_fallback(self):
        ...
        assert title == "Discussion", (
            "Devrait fallback vers 'Discussion' si pas de mots significatifs"
        )
    ...
    async def test_grouping_integration(self, mock_vector_service):
        ...
        assert len(groups) >= 1, "Au moins 1 groupe doit être créé"
        assert sum(len(g) for g in groups.values()) == 5, (
            "Tous les concepts doivent être assignés"
        )
        ...
        for group_id, group_concepts in groups.items():
            if group_id != "ungrouped":
                ...
                assert title is not None, (
                    f"Titre ne doit pas être None pour groupe {group_id}"
                )
                assert len(title) > 0, (
                    f"Titre ne doit pas être vide pour groupe {group_id}"
                )
                assert "utilisateur" not in title.lower(), (
                    "Titre ne doit pas contenir de stop words"
                )
    ...
    async def test_grouping_performance(self, mock_vector_service):
        ...
        assert duration_ms < 500, (
            f"Groupement trop lent: {duration_ms:.0f}ms (attendu < 500ms)"
        )
